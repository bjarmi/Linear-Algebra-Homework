...
def create_voting_dict(strlist):
    ...
    """
    return {
        x[0]: [int(i) for i in x[3:]] for x in [y.split(' ') for y in strlist]
    }
...
def most_similar(sen, voting_dict):
    ...
    """
    current: tuple = ("", float('-infinity'))
    for i in voting_dict:
        if i == sen:
            continue
        comp = policy_compare(sen, i, voting_dict)
        if current[1] < comp:
            current = (i, comp)
    return current[0]
...
def read_voting_records(filename="voting_record_dump109.txt"):
    try:
...
records: dict = create_voting_dict(read_voting_records())
most_like_chafee = most_similar("Chafee", records)
least_like_santorum = least_similar("Santorum", records)
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        >>> vd = {'Klein':[1,1,1], 'Fox-Epstein':[1,-1,0], \
            'Ravella':[-1,0,0],'Oyakawa':[-1,-1,-1], 'Loery':[0,1,1]}
        >>> sens = {'Fox-Epstein','Ravella','Oyakawa','Loery'}
        >>> find_average_similarity('Klein', sens, vd)
        -0.5
    """
    return sum(
        [policy_compare(sen, x, voting_dict) for x in sen_set]
    ) / len(sen_set)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        >>> voting_dict = {'Klein': [-1,0,1], 'Fox-Epstein': [-1,-1,-1], \
            'Ravella': [0,0,1]}
        ...
        >>> senators
        {'Fox-Epstein','Ravella'}
        >>> d = {'c': [-1,-1,0], 'b': [0,1,1], 'a': [0,1,1], 'e': [-1,-1,1], \
            'd': [-1,1,1]}
    ...
    sen_list_len: int = len(sen_list)

    sum_votes = voting_dict[sen_list[0]]
    for i in range(1, sen_list_len):
        sum_votes = [
            a + b for (a, b) in zip(sum_votes, voting_dict[sen_list[i]])
        ]

    return [x / sen_list_len for x in sum_votes]
...
def get_all_possible_pairs(iterable):
    all_possible_pairs = []
    for x in iterable:
        for y in iterable:
            if x == y:
                continue
            if (y, x) not in all_possible_pairs:
...
def bitter_rivals(voting_dict):
    ...
    Example: 
        ...
        True
    """
    all_possible_pairs = get_all_possible_pairs(voting_dict)

    current_rivals = tuple()
    current_least_similar_score = float('infinity')

    for sen1, sen2 in all_possible_pairs:
        comp = policy_compare(sen1, sen2, voting_dict)

        if current_least_similar_score > comp:
            current_rivals = (sen1, sen2)
            current_least_similar_score = comp
    return current_rivals
